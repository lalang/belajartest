                    ...
                    <tbody><tr bgcolor="#fff">
                      <th style="width: 10px">#</th> 
                        ...
                        foreach ($perizinan_berkas as $value) {
                            ...
                            $model_bi = BerkasIzin::findOne(['id' => $value->berkas_izin_id]);
                            ...
                            <tr bgcolor="#fff">
                                <td><?= $value->urutan ?></td>
                                <td><?= $value->berkasIzin->nama ?> Format: (<?= $model_bi->extension ?>)<?= $text ?></td>
                                <td>
                                    <?= Html::dropDownList('user_file[]', $value->user_file_id, ArrayHelper::map(UserFile::find()->where('user_id=' . Yii::$app->user->id)->andFilterWhere(['or', $filter])->all(), 'id', 'description'), ['prompt' => '--Pilih--', 'class' => 'form-control '.$clas, 'id' => 'fileForm2', 'onchange' => 'submitChange2()']) ?>
                                </td>
